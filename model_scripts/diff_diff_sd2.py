...
# Borrowed from https://github.com/exx8/differential-diffusion/blob/main/SD2/diff_pipe.py
...
class StableDiffusionDiffImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if image.shape[1] == 4:
            map = torchvision.transforms.Resize(tuple(s for s in image.shape[2:]),antialias=None)(map)
        else:
        ...
        if output_type == "pt":
            image = [image[i].permute(2, 0, 1).to(latents) for i in range(image.shape[0])]
        ...
        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)# Copyright 2023 The HuggingFace Team. All rights reserved.
